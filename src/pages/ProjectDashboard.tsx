...
export default function ProjectDashboard() {
  ...
  if (!selectedOrganizationId) {
    return (
      <Page>
        ...
        <PageHeader title="Project Dashboard" description="Choose a workspace first. Project tools stay locked until you explicitly select an organization or the default workspace." />
  ...
  if (!projects.data?.projects?.length) {
    return (
      <Page>
        <PageHeader title="Project Dashboard" description="No projects are available in the selected workspace." />
  ...
  if (!selectedProjectId || !proj) {
    return (
      <Page>
        ...
        <PageHeader title="Project Dashboard" description={`Select a project from ${selectedWorkspaceName || "this workspace"}.`} />
        <div className="grid md:grid-cols-2 gap-3">
          {projects.data.projects.map((project: any) => (
            <button key={project.id} onClick={() => { setSelectedProjectId(project.id); playUiSound("nav"); navigate("/project/dashboard"); }} className="hairline rounded-lg bg-card p-4 text-left hover:bg-accent/40 transition-colors">
  ...
  return (
    <Page>
      ...
      <PageHeader title="Project Dashboard" description={proj?.name ? `${proj.name}` : "Select a project"} />
